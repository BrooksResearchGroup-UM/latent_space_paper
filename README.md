...
To pre-process MSA files and perform one-hot encoding for each dataset, use this [jupyter notebook](./notebooks/MSA.ipynb).
...
SSNs were generated by [EFI - Enzyme Similarity Tool](https://efi.igb.illinois.edu/efi-est/), submitted through Option C - FASTA with the default setting. Preliminary results were assigned customized alignment score (AS) for downstream clustering. Clustering outputs (XGMML) were downloaded, reformatted using this [jupyter notebook](./notebooks/ssn_analysis.ipynb), and visualized by [Cytoscape](https://cytoscape.org/). Clustering results were saved as CSV files for analysis.